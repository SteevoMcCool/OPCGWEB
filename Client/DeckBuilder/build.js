...
let currDeck = [];  //list of {count:num, uid:num, art:num}
...
function deckToText(deck,leader){
    let sf = (leader) ? `1x ${cardList[leader.uid].set}-${cardList[leader.uid].iid} (L)\n` :''
    deck.forEach(cm=>sf += `${cm.count}x ${cardList[cm.uid].set}-${cardList[cm.uid].iid}\n`)
    return sf;
}
function stringToDeck(str){
    ...
    for (let i = 0; i < str.length; i+=6){
        ...
        let id262144 = alphabet.indexOf(str[i+2]);
        let id4096 = alphabet.indexOf(str[i+3]);
        let id64 = alphabet.indexOf(str[i+4]);
        let idu = alphabet.indexOf(str[i+5]);
        let uid = id262144*262144 + id4096*4096 + id64*64+ idu
        if (count) deck.push({count,uid,art})
        else lead = {uid,art}
    }
    return {deck, lead}
}
function addCardToDeck(uid,art){
    let i = currDeck.findIndex(c=> c.uid == uid);
    if (i == -1) { 
        currDeck.push({count:1,uid,art});  
        return currDeck = currDeck.sort((a,b) => cardList[a.uid].cost - cardList[b.uid].cost);
    }
    currDeck[i].count++;
    currDeck[i].art = art;
}

function remCardFromDeck(uid,art){
    let i = currDeck.findIndex(c=> c.uid == uid)
    if (i == -1) return false;
    if (currDeck[i].count == 1){
        currDeck = currDeck.filter(c => c.uid != uid)
        return true;
    }
    else currDeck[i].count--;
    return false;
}
function numCardInDeck(uid){
    let i = currDeck.findIndex(c=> c.uid == uid);
    if (i == -1) return 0;
    return currDeck[i].count;
}
let currLeader =  false //obj of {uid:num, art:num}
...
function loadCards(s = (a,b)=> a.cost - b.cost){
    ...
    l.forEach(c => {
        sf += 
        `
            <div style="background-image:url('${c.imgs[0]}')" id="${c.uid}-0"> 
    ...
    id('cardzone').querySelectorAll("div").forEach(cdiv=>{
        ...
        cdiv.onmouseenter = e =>{
            ...
            cdiv.querySelector('#d_use').onclick = e =>{
                if (type == "Leader") {
                    currLeader = {uid,art};
                    cw.SETTO(stringToColArr(cardList[uid].color))
...
function loadDeck() {
    ...
    if (currLeader){
        let lcard = cardList[currLeader.uid]
    ...
    id('maindeck').querySelectorAll('.cardInMainDeck').forEach(cdiv=> {
        cdiv.oncontextmenu = (e)=>{
            ...
            e.preventDefault();
            let [uid, art] = cdiv.id.split('-')
            let x = remCardFromDeck(uid);
            if (x) cdiv.remove()
            else{
                let ncid =  numCardInDeck(uid)
        ...
        cdiv.onmouseenter = () => {
            let [uid, art] = cdiv.id.split('-')
            id('impCard').style.backgroundImage = `url(${cardList[uid].imgs[art]})`
        }
        cdiv.onmouseleave = () => {
            id('impCard').style.backgroundImage = '';
        }
...
id('share').onclick = ()=> {
    ...
    id('copyLink').onclick = ()=> copyToClip(`${host}/build?ld=${deckToString(currDeck,currLeader)}`)+ exit.onclick();
        ...
        user.decks.forEach( (deck,i) =>{
            ...
            `     
                ...
                sf += `
                    <div id='deckSlot${i}'>
                        <center>
                        <a class='captian gbg' style='background-image:url("${cardList[lead.uid].imgs[lead.art]}"); display:block' href='./build?fcu=${i}'>
                        </a>
        ...
        id('sbu').onclick = () => {
            ...
            id('savedeckbu').onclick = async () =>{
                ...
                let res = await fetch('/setDeck',{
                    method:'post',
                    body: JSON.stringify({id:user.id, session:user.session, slot,deckName,deckString})
                })
                console.log("GOOD");
                ...
                if (resj.good) window.location.replace(`/build?fcu=${slot}`)
            }
        }
